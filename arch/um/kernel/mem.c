...
#ifdef CONFIG_NUMA
	/*
	 * Assign all memory to node 0 so free_area_init() finds it when
	 * initializing the single node. NODE_DATA(0) itself is allocated
	 * by free_area_init() via alloc_offline_node_data().
	 */
