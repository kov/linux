...
void userspace(struct uml_pt_regs *regs)
{
	...
	while (1) {
		...
		if (using_seccomp) {
...
#endif
			if (sig == SIGSEGV)
				os_get_faultinfo(mm_id->pid, &regs->faultinfo,
						 si, regs);
		} else {
			int pid = mm_id->pid;
...
#ifdef __aarch64__
			if (ptrace_set_tls(pid, regs->tpidr_el0)) {
				printk(UM_KERN_ERR
				       "%s - ptrace set tls failed, errno = %d\n",
				       __func__, errno);
				fatal_sigsegv();
			}
#endif
			...
			os_arch_process_handshake(pid, regs,
						  (void *)mm_id->stack);
			...
			if (WIFSTOPPED(status) &&
				...
				int st2;
				unsigned long pending_sigs = 0;

				kill(pid, SIGSTOP);
				ptrace(PTRACE_CONT, pid, 0, 0);

				/*
				 * Wait for our SIGSTOP.  Other signals may
				 * arrive first — suppress them, record in a
				 * bitmask, and re-inject after the loop so
				 * we don't spin.
				 */
				CATCH_EINTR(err = waitpid(pid, &st2,
							  WUNTRACED | __WALL));
				while (WIFSTOPPED(st2) &&
				...
				/* Re-inject any signals that arrived before
				 * our SIGSTOP */
				while (pending_sigs) {
					int s = __builtin_ctzl(pending_sigs);
			...
			os_arch_post_wait_handshake(pid, regs);
			...
			case SIGILL:
#ifdef __aarch64__
				/* SVE/PAC emulation or skipping */
				{
					unsigned long pc = regs->gp[32];
					unsigned int insn = 0;

					/* Read instruction at PC */
					errno = 0;
					insn = ptrace(PTRACE_PEEKTEXT,
						      mm_id->pid, pc, 0);
					if (errno == 0) {
						/* Check for PAC hints */
						/* PACIASP: d503233f, AUTIASP: d503237f */
						/* PACIBSP: d50323bf, AUTIBSP: d50323ff */
						/* Also generic PACIAZ, etc... check just the hint encoding? */
						/* Hint space: d5032xxx */
						if ((insn & 0xfffff01f) ==
						    0xd503201f) {
							/* Is a hint.
							 * 0x1f = NOP
							 * 0x3f, 0x7f, 0xbf, 0xff = PAC
							 */
							unsigned int op2 =
								(insn >> 5) &
								0x7;
							unsigned int crm =
								(insn >> 8) &
								0xf;
							/* PAC encoding: CRm=0011 (3), Op2=001,011,101,111 */
							if (crm == 3 &&
							    (op2 & 1)) {
								/* Emulate as NOP: Skip instruction */
								regs->gp[32] +=
									4;

								/* If AUT instruction (Op2 5 or 7), strip PAC signature from LR (x30) */
								if (op2 & 4) {
									regs->gp[30] &=
										0x0000ffffffffffffULL;
								}

								/* Loop around to update child regs and resume */
								sig = 0;
								break;
							}
						}
		...
		if (sig) {
			...
			case SIGTRAP:
				if (using_seccomp || PTRACE_FULL_FAULTINFO)
					...
					segv(regs->faultinfo, 0, 1, regs, NULL);
				break;
			case SIGIO:
			case SIGILL:
			case SIGBUS:
