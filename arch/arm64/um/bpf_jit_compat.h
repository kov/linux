 ...
 *
 * The capability numbers come from the generated cpucap-defs.h and
 * alternative_has_cap_likely() from alternative-macros.h works as-is
 * (alternatives are never applied on UML, so it is always false).
 * UML's asm/cpufeature.h does not provide cpus_have_cap(), though.
 */
